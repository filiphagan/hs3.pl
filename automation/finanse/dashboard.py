...
def export_dashboard_cashflow(source_path=".", offline=False, standalone=False):
    ...
    try:
        ...
        print(f"Source file {source_file} not found. Unable to create dashboard without input data.")
    ...
    fig.add_trace(go.Scatter(x=x, 
                             y=(df.Przychody-df.Rozchody).cumsum(),
...
def update_html(source_path):
    ...
    with open(os.path.join(source_path, OUTPUT_FN), encoding="utf-8") as index:
    ...
    with open(os.path.join(source_path, OUTPUT_FN), mode="w", encoding="utf-8") as output:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--standalone", action="store_false")
    ...
    print(f"{OUTPUT_FN} updated successfully")
